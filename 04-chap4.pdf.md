...
Using the null plots, that is, when $D=0$, we can estimate the probability of rejecting $H_0$ when $H_0$ is true, which we denote as $\hat\alpha$, for each plot type.
