...
# source("paper/gen_functions.R")
# Source function for generating data
...
Aust_map <- ozmap_data(data = "states", quiet = TRUE) |>
  group_by(NAME) |>
  mutate(value = dist_normal(mu = sample(seq(0,8), 1), sample(seq(0,4), 1))) |>
  ungroup() |>
  mutate(bitemp = cut(mean(value), breaks=seq(0,8), labels=seq(1,8)),
         bivar = cut(sqrt(variance(value)), breaks=seq(0,4), labels=seq(1,4)),
         biclass = paste(bitemp, bivar, sep="-")) 

# CHOROPLETH MAP
m1 <- Aust_map |>
  ggplot() +
  geom_sf(aes(fill = mean(value), geometry = geometry),
          colour="black") + 
  scale_fill_davos() +
  labs(fill = "Value") +
  theme_map() +
  ...
  ggtitle("Choropleth") + 
  scale_x_continuous(limits = c(111,155))
...
m2 <-  Aust_map |>
  ggplot() +
  geom_sf_sample(aes(fill = value, geometry = geometry), 
                 colour=NA, times=n, seed=5) + 
  geom_sf(aes(fill = NA, geometry = geometry), colour="black") + 
  scale_fill_davos() +
  labs(fill = "Value") +
  theme_map() +
  theme(legend.position = "inside", 
...
m3 <-  Aust_map |>
  ggplot() +
  geom_sf_sample(aes(fill = value, geometry = geometry), 
                 alpha = 1.2/n, colour=NA, times=n,
                 position = "identity", seed=5) + 
  geom_sf(aes(fill = NA, geometry = geometry), colour="black") + 
  scale_fill_davos() +
  labs(fill = "Value") +
  theme_map() +
...
# BIVARIATE MAP
# colour suppression function

# Make base palette
basecols <- color("davos")(8)
...
b1 <- Aust_map |>
  ggplot() +
  geom_sf(aes(fill = biclass, geometry = geometry), colour="black") + 
...
m4 <-  b1 + inset_element(b2, left = -0.01, bottom = -0.5, 
                          right = 0.6, top = 0.4)
...
# make palette
v2 <- b2 + scale_fill_manual(values = VSUP)

# final vsup plot
m5 <- v1  + inset_element(v2, left = -0.01, bottom = -0.5, 
                          right = 0.6, top = 0.4)
...
#| message: false
#| eval: false
ggsave(plot = m1 + labs(title = NULL) + theme(legend.position="none"),
       filename ="paper/images/choro.png",
       dpi = "screen")
ggsave(plot = m2 + labs(title = NULL) + theme(legend.position="none"),
       filename ="paper/images/pixel.png",
       dpi = "screen")
ggsave(plot = m3 + labs(title = NULL) + theme(legend.position="none"),
       filename ="paper/images/trans.png",
       dpi = "screen")
ggsave(plot = b1 + labs(title = NULL) + theme(legend.position="none"),
       filename ="paper/images/bivar.png",
       dpi = "screen")
ggsave(plot = v1 + labs(title = NULL) + theme(legend.position="none"),
       filename ="paper/images/vsup.png",
       dpi = "screen")
```
...
gog <- tibble(
  "Name" = c("Choropleth", "Bivariate", "VSUP", 
             "Pixel", "Transparency"),
  #"Data type" = c(rep("Distribution",5)),
  "Distributional Statistic" = c("Mean", "Mean & Variance", "Mean & Variance", 
                                 "Sample", "Sample"),
  "Geometric Position" = c("Identity", "Identity", "Identity", 
                           "Subdivide", "Transparency"),
  "Coordinate (colour)" = c("Cartesian", "Cartesian", "Warped Cartesian", "Cartesian", "Cartesian"),
  "Aesthetic (colour)" = c("Hue", "Hue & Saturation", "Hue & Saturation", 
                           "Hue", "Hue")
)  

# in console
# img <- c("paper/images/choro.png",
#           "paper/images/bivar.png",
#           "paper/images/vsup.png",
#           "paper/images/pixel.png",
#           "paper/images/trans.png")
# in knit
img <- c("images/choro.png",
          "images/bivar.png",
          "images/vsup.png",
          "images/pixel.png",
          "images/trans.png")

gog_table <- cbind(img, gog) |>
  flextable() 

gog_tbl <- colformat_image(x = gog_table,
                           i=1:5,
                           j = "img", 
                           width = 2, 
                           height = 1,
...
(m1 | m4 | m5 | m2 | m3) / wrap_flextable(gog_tbl, flex_cols = TRUE, flex_body = TRUE)
...
parent_dists <- expand_grid(D, V) |>
  mutate(out = dist_normal(-0.5*D, 1),
         `in` = dist_normal(0.5*D, 1)) |>
    pivot_longer(cols = out:`in`,
               names_to = "group",
               values_to = "dist")
...
dist_5x5 <- parent_dists |>
  ...
  stat_slab(data = estimate_dists, normalize = "panels",
            fill=NA, linewidth=0.1, alpha=0.7) + 
  geom_hline(yintercept = 0, colour = "black") +
  
  # Visualise Cumultive Distributions
  # Haven't done yet
...
ggsave(plot = dist_5x5,
       filename ="paper/images/dist_5x5.png",
       width = 1800,
       height = 1800,
       units = "px",
       dpi = "screen")
...
dl <- df |>
  mutate(D = factor(D, labels = paste("Distance =", Dlab)),
         V = factor(V, labels = paste("Varince =", Vlab))) |>
  dist_list()
local_dist_5x5 <- ((dl[[5]] | dl[[10]] | dl[[15]] | dl[[20]] | dl[[25]])/
    (dl[[4]] | dl[[9]] | dl[[14]] | dl[[19]] | dl[[24]])/
    (dl[[3]] | dl[[8]] | dl[[13]] | dl[[18]] | dl[[23]])/
    (dl[[2]] | dl[[7]] | dl[[12]] | dl[[17]] | dl[[22]])/
    (dl[[1]] | dl[[6]] | dl[[11]] | dl[[16]] | dl[[21]]))

ggsave(plot = local_dist_5x5,
       filename ="paper/images/localdist_5x5.png",
       width = 1800,
       height = 1800,
       units = "px",
       dpi = "screen")
...
# make pixel map
ps <- pixel_list(df, times = 50)
pix <- ((ps[[5]] | ps[[10]] | ps[[15]] | ps[[20]] | ps[[25]])/
    (ps[[4]] | ps[[9]] | ps[[14]] | ps[[19]] | ps[[24]])/
    (ps[[3]] | ps[[8]] | ps[[13]] | ps[[18]] | ps[[23]])/
    (ps[[2]] | ps[[7]] | ps[[12]] | ps[[17]] | ps[[22]])/
    (ps[[1]] | ps[[6]] | ps[[11]] | ps[[16]] | ps[[21]]))

ggsave(plot = pix,
       filename ="paper/images/pix_5x5.png",
       width = 1800,
       height = 1800,
       units = "px",
       dpi = "screen")
...
# Make Choropleth map
ps <- choropleth_list(df)
choromap <- ((ps[[5]] | ps[[10]] | ps[[15]] | ps[[20]] | ps[[25]])/
    (ps[[4]] | ps[[9]] | ps[[14]] | ps[[19]] | ps[[24]])/
    (ps[[3]] | ps[[8]] | ps[[13]] | ps[[18]] | ps[[23]])/
    (ps[[2]] | ps[[7]] | ps[[12]] | ps[[17]] | ps[[22]])/
    (ps[[1]] | ps[[6]] | ps[[11]] | ps[[16]] | ps[[21]]))

ggsave(plot = choromap,
       filename ="paper/images/choro_5x5.png", 
       width = 1800, 
       height = 1800,
       units = "px",
       dpi = "screen")
...
# Make Bivar map
ps <- bivar_list(df)
bivarmap <- ((ps[[5]] | ps[[10]] | ps[[15]] | ps[[20]] | ps[[25]])/
    (ps[[4]] | ps[[9]] | ps[[14]] | ps[[19]] | ps[[24]])/
    (ps[[3]] | ps[[8]] | ps[[13]] | ps[[18]] | ps[[23]])/
    (ps[[2]] | ps[[7]] | ps[[12]] | ps[[17]] | ps[[22]])/
    (ps[[1]] | ps[[6]] | ps[[11]] | ps[[16]] | ps[[21]]))

ggsave(plot = bivarmap,
       filename ="paper/images/bivar_5x5.png", 
       width = 1800, 
       height = 1800,
       units = "px",
       dpi = "screen")
...
# Make VSUP map
ps <- VSUP_list(df)
vsupmap <- ((ps[[5]] | ps[[10]] | ps[[15]] | ps[[20]] | ps[[25]])/
    (ps[[4]] | ps[[9]] | ps[[14]] | ps[[19]] | ps[[24]])/
    (ps[[3]] | ps[[8]] | ps[[13]] | ps[[18]] | ps[[23]])/
    (ps[[2]] | ps[[7]] | ps[[12]] | ps[[17]] | ps[[22]])/
    (ps[[1]] | ps[[6]] | ps[[11]] | ps[[16]] | ps[[21]]))
...
ggsave(plot = vsupmap,
       filename ="paper/images/vsup_5x5.png", 
       width = 1800, 
       height = 1800,
       units = "px",
       dpi = "screen")
...
ps <- transparency_list(df, times = 50)
transmap <- ((ps[[5]] | ps[[10]] | ps[[15]] | ps[[20]] | ps[[25]])/
    (ps[[4]] | ps[[9]] | ps[[14]] | ps[[19]] | ps[[24]])/
    (ps[[3]] | ps[[8]] | ps[[13]] | ps[[18]] | ps[[23]])/
    (ps[[2]] | ps[[7]] | ps[[12]] | ps[[17]] | ps[[22]])/
    (ps[[1]] | ps[[6]] | ps[[11]] | ps[[16]] | ps[[21]]))

ggsave(plot = transmap,
       filename ="paper/images/trans_5x5.png", 
       width = 1800, 
       height = 1800,
       units = "px",
       dpi = "screen")
...
![Theoretical distributions](images/dist_5x5.png){#fig-dist25 width=90%} 

![Chropleth map](images/choro_5x5.png){#fig-choro25 width=90%} 

![Bivariate map](images/bivar_5x5.png){#fig-vsup25 width=90%} 

![VSUP map](images/vsup_5x5.png){#fig-vsup25 width=90%}  

![Pixel map](images/pix_5x5.png){#fig-pix25 width=90%} 

![Transparency map](images/trans_5x5.png){#fig-trans25 width=90%} 
...
![](images/check0.png){#fig-chor width=85%}

![](images/check1.png){#fig-chor width=85%}

![](images/check2.png){#fig-chor width=85%}


![](images/check3.png){#fig-chor width=85%}

![](images/check4.png){#fig-chor width=85%}

![](images/check5.png){#fig-chor width=85%}

![](images/check6.png){#fig-chor width=85%}

![](images/check7.png){#fig-chor width=85%}

![](images/check8.png){#fig-chor width=85%}

![](images/check9.png){#fig-chor width=85%}
...
durations <- study_data |>
  group_by(id) |>
  na.omit() |>
  summarise(total_duration = as.numeric(sum(duration))/60)
...
number_assigned_randomly <- chisq.test(table(study_data$correct_number, study_data$plot_type))

```

`r length(unique(study_data$id))` individuals completed the study and passed the attention check.
The median study duration was `r round(median(durations$total_duration),2)` minutes with an interquartile range of `r round(IQR(durations$total_duration),2)` minutes.
...
A Pearson's chi-squared test for independence indicates a non-significant relationship between plot type and number present in the data, indicating that random assignment of numbers was successful ($p$-value of `r round(number_assigned_randomly$p.value, 2)`).

```{r demographics}
#| echo: false

demo_data <- study_data %>% select(id, age, pronouns, education, colourblind, continent) %>% distinct()

```

Demographically, participants tended to be younger, use he/him pronouns, and have a tertiary education.
`r round(sum(demo_data$age=="18 - 25")/nrow(demo_data)*100, 2)`% of individuals identified as being between 18 and 25, while `r round(sum(demo_data$age=="26 - 35")/nrow(demo_data)*100, 2)`% identified as 26 to 35; the remaining individuals identified as older than 35.
`r round(sum(demo_data$pronouns=="He/His")/nrow(demo_data)*100,2)`% of individuals use he/him pronouns, `r round(sum(demo_data$pronouns=="She/Her")/nrow(demo_data)*100,2)`% use she/her, and the remaining individuals use they/them, or prefer not to answer.
`r round(sum(demo_data$education=="Tertiary education")/nrow(demo_data)*100, 2)`% of individuals indicated that their highest education level was tertiary, while the remaining individuals identified their education level as below tertiary or post-secondary non-tertiary education.
A large number of participants were from Africa and Europe (`r round(sum(demo_data$continent=="Africa", na.rm=TRUE)/nrow(demo_data)*100,2)`% and `r round(sum(demo_data$continent=="Europe", na.rm=TRUE)/nrow(demo_data)*100,2)`%, respectively), and `r round(sum(demo_data$colourblind=="Yes/Unsure")/nrow(demo_data)*100,2)`% of participants indicated that they were colourblind or unsure.
...
# Get data with no visible number
h0_true <- study_data |>
...
combined <- NULL
for (i in 1:b) {
  for (j in levels(d0_data$plot_type)) {
  # draw bootstrapped sample
    ...
    boot_alpha <- dsamp |>
      ...
      #group_by(plot_type) |>
      mutate(bcorrect = sample(correct, replace = TRUE)) |>
      #summarise(alpha = 1 - mean(correct)) |>
      summarise(alpha = length(bcorrect[!bcorrect])/length(bcorrect)) |>
      mutate(run = i, 
             plot_type = j)
    combined <- rbind(combined, boot_alpha)
  }
}

# save(combined, file="paper/data/combined.rda")
...
# Calculate accept/reject based on cut off
df2 <- bigpvalues |>
  group_by(test) |>
  mutate(chor = pvalue <= h0_true$alpha[1],
         biv = pvalue <= h0_true$alpha[2],
         vsup = pvalue <= h0_true$alpha[3],
         pix = pvalue <= h0_true$alpha[4],
         over = pvalue <= h0_true$alpha[5]) |>
  pivot_longer(cols = chor:over,
               values_to = "reject",
               names_to = "plot_type") |>
  mutate(plot_type = factor(plot_type,  
                            levels = c("chor", "biv", "vsup", "pix", "over"),
                            labels = c("Choropleth", "Bivariate", "VSUP", 
                                       "Pixel", "Transparency"))) |>
  ungroup() |>
  rename("correct_number" = num)

# get filtered data for each test type for calculations
filtered_ttest <- df2 |>
  filter(test=="t-test")
...
# Make glmer model for each test
ttest_model <- glmer(reject ~ D*V*plot_type +  (1|correct_number),
                     family="binomial", 
                     data = filtered_ttest)


mtest_model <- glmer(reject ~ D*V*plot_type +  (1|correct_number),
                     family="binomial", 
                     data = filtered_mtest)

# Make table with predictions
ttest_pable <- filtered_ttest |>
  mutate(power_fixed = predict(ttest_model, type = "response", re.form = NA),
         power_random = predict(ttest_model, type = "response"))
mtest_pable <- filtered_mtest |>
  mutate(power_fixed = predict(mtest_model, type = "response", re.form = NA),
         power_random = predict(mtest_model, type = "response"))

# combine t-test and mtest
theoretical_models <- rbind(ttest_pable, mtest_pable)  |>
  mutate(test = factor(test, levels = c("t-test", "m-test"),
                       labels = c("T-test", "Moran's I")))

save(theoretical_models, file="paper/data/theoretical_models.rda")
# save(theoretical_models, file="data/theoretical_models.rda")

# Check theoretical model makes sense
# check fixed effects
# theoretical_models |>
#   ggplot(aes(x=V)) +
#   geom_point(aes(y = as.numeric(reject)),  size=0.1,
#              position = position_jitter(width=0.2, height=0.1)) +
#   geom_line(aes(y = power_fixed, group = interaction(test, plot_type))) +
#   facet_grid(rows = vars(test), cols = vars(D))
...
#   geom_point(aes(y = as.numeric(reject)),  size=0.1,
#              position = position_jitter(width=0.2, height=0.1)) +
#   geom_line(aes(y = power_random, 
#                 group = interaction(test, plot_type, correct_number))) +
#   facet_grid(rows = vars(test), cols = vars(D))

## Also average case
average_tests <- bigpvalues |>
  mutate(reject = pvalue <= mean(h0_true$alpha))

average_ttest <- glm(reject ~ D*V,
                     family="binomial",
                     data = filter(average_tests, test == "t-test")) |>
  augment(type.predict="response") |>
  mutate(test = "t-test")

average_mtest <- glm(reject ~ D*V,
                     family="binomial", 
                     data = filter(average_tests, test == "m-test")) |>
  augment(type.predict="response") |>
  mutate(test = "m-test")

average_fit <- rbind(average_ttest, average_mtest) |>
  rename("power"= .fitted) |>
  mutate(test = factor(test, levels = c("t-test", "m-test"),
                       labels = c("T-test", "Moran's I")))
...
save(average_fit, file="paper/data/average_fit.rda")
...
#| message: false
#| out-width: 100%
#| fig-cap: "The full set of participant responses, coloured by whether or not they were able to identify the correct number in the plot, grouped according to the distance ($D$) between the distributions, and the standard deviation ($V$) in each individual estimate. The theoretical simulation from the $t$-test and Moran's I calculations is also included as a point of reference."
...
study_data |>
  ...
  rbind(theory_data_plot) |>
  ggplot(aes(x=D, y=V))+
  geom_jitter(height=0.3, width=0.3, alpha=0.5, size=0.5, 
              aes(colour = correct_original)) +
  facet_wrap(vars(plot_type), nrow=2) +
  scale_colour_manual(values = c(color("high contrast")(2), "black")) +
  theme_bw() +
...
#| message: false
#| out-width: 80%
#| fig-cap: "Bootstrapped significance levels for each plot type, shown as jittered dotplots, computed using the null plots, $D=0$. The large points are the full sample values, that are used in the power analysis. The vertical line indicates the conventional significance level, 0.05."

load("data/combined.rda")
# p1 <- combined |>
#   group_by(plot_type) |>
#   summarise(lower = quantile(alpha, probs = 0.025),
#             upper = quantile(alpha, probs = 0.975),
#             average = mean(alpha)) |>
#   ggplot() +
#   geom_errorbar(aes(xmin=lower, xmax = upper, y = plot_type, colour = plot_type),
#                 width = 0.4) +
#   geom_point(aes(x=average, y = plot_type, colour = plot_type)) +
#   theme_classic() +
#   theme(aspect.ratio = 1,
#         legend.position = "none") +
...
p1 <- combined |>
  ...
  geom_quasirandom(aes(x = alpha, y = plot_type, colour = plot_type), alpha=0.01) +
...
#   add_header_row(
#     colwidths = c(2, 2),
#     values = c("", "Number of Responses")) |>
#   set_header_labels(
#     plot_type = "Plot Type",
#     alpha = "Significance",
#     n_correct = "No number visibile",
#     n_reject = "Guessed number"
#     ) |>
#   colformat_double(
#   decimal.mark = get_flextable_defaults()$decimal.mark,
#   digits = 3,
# ) |>
#   wrap_flextable(flex_cols = TRUE, flex_body = TRUE)
...
#| warning: false
#| message: false
# Make generalised linear model (basic)
# glm_model <- glm(correct ~  D*V*plot_type,
#                    family="binomial", 
#                    data = h1_true)

# Slow to run, so save and load
if (file.exists("data/glmer_models.rda")) {
  load("data/glmer_models.rda")
} else {
# Make generalised linear model (random effects)
  glmer_model <- glmer(correct_original ~  D*V*plot_type +    (1|correct_number) + (1|id),
                   family="binomial", 
                   data = study_data)

  glmer_models <- study_data |>
    mutate(power_fixed = predict(glmer_model, type = "response", re.form = NA),
         power_random = predict(glmer_model, type = "response"))
  save(glmer_models, file="data/glmer_models.rda")
}
```
...
#| fig-height: 13
#| out-width: 100%
#| fig-cap: "Visualisation of generalised linear random effects model (top) generalised linear fixed effects model (bottom)."
...
p_fe <- glmer_models |>
  filter(!D==0) |>
  ggplot(aes(x=V)) + 
  geom_line(aes(y = power_fixed, group = plot_type, 
                colour = plot_type), 
            linewidth=1, alpha=0.7) +
  geom_line(data = average_fit |> filter(!D==0), 
            aes(y=power, group=test, linetype = test), 
            linewidth=1.5, colour = "black", alpha=1) +
  facet_grid(rows = vars(test), cols = vars(D),
             labeller = labeller(D = name_distance)) +
  #facet_wrap(~D, nrow=1, labeller = labeller(D = name_distance)) +
  theme(legend.position="bottom",
        legend.direction = "horizontal", text=element_text(size=10), strip.text.x=element_text(size=8),
        strip.text.y=element_text(size=8),
        legend.text = element_text(size=8)) +
  guides(colour = guide_legend(nrow = 2))+
  labs(x = "Standard Deviation", y = "Power", colour = "Plot type",
       linetype = "Test") +
  guides(linetype = "none") +
  scale_linetype_manual(values = LINES) 

p_re / p_fe + plot_layout(heights = c(3, 1))
```
...
```{r}
#| label: tbl-v-trend
#| tbl-cap: "Effect of Standard Deviation (V) by Plot Type, Averaged Over Distance (D)"
#| echo: false
#| eval: true
#| message: false
#| cache: true

if (file.exists("data/glmer_model.rda")) {
  load("data/glmer_model.rda")
} else {
  glmer_model <- glmer(correct_original ~  D*V*plot_type +    (1|correct_number) + (1|id),
                   family="binomial", 
                   data = study_data)
  save(glmer_model, file="data/glmer_model.rda")
}
emtrends(glmer_model, "plot_type", var = "V", infer=TRUE, at = list(D=c(1,2,3,4))) |> as.data.frame() %>%
  select(plot_type, V.trend, SE, z.ratio, p.value) %>%
  kable(#caption="Results for Standard Deviation Trend by Plot Type, Averaged Over Distance",
        digits=3, format="latex", longtable=FALSE, #label="vtrend",
        col.names=c("Plot Type", "V Effect", "SE", "Z Ratio", "P-Value")) 
...
```{r}
#| label: tbl-basicmodel
#| tbl-cap: "Selected Comparison of Plot Types for Standard Deviation ($V$), Averaged Over Distance"
#| echo: false
#| eval: true
#| message: false

compare2 <- pairs(emtrends(glmer_model, "plot_type", var = "V", infer=TRUE, at = list(D=c(1,2,3,4), plot_type=c(c("Transparency", "Pixel")))))

compare1 <- pairs(emtrends(glmer_model, "plot_type", var = "V", infer=TRUE, at = list(D=c(1,2,3,4), plot_type=c(c("Choropleth", "Bivariate")))))

# emtrends(glmer_model, pairwise ~ "plot_type", var = "V", infer=TRUE, at = list(D=c(1,2,3,4)), adjust="Tukey")$contrasts %>% as.data.frame() 

rbind(compare1,compare2) %>% as.data.frame() %>% select(contrast, estimate, SE, z.ratio, p.value) %>% 
  kable(#caption="Selected Comparison of Plot Types for Standard Deviation Trend, Averaged Over Distance", 
    digits=3, format="latex", longtable=FALSE, #label="pairwisev",
    col.names=c("Contrast", "Estimate", "SE", "Z Ratio", "P-Value"))

# Note: Setting different V's results in different pairwise significance (may want to include multiple levels in discussion)
# emtrends(glmer_model, pairwise ~ "plot_type", var = "D", infer=TRUE, at = list(V=2), adjust="Tukey")$contrasts %>%
#   kable(caption="Results for Distance Trend by Plot Type, Averaged Over Standard Deviation", digits=3)
...
library(spelling)
qmd <- "paper/experiment-paper-formatting.qmd"
ignore <- readLines("paper/WORDLIST")
...
# make 6 data set
set.seed(6)
six_data <- gen_data(3, 3, parentV = 1, noiseV = 0, number = 6)
...
# a <- theme(plot.title = element_text(size = 24, 
#                                      face = "bold",
#                                      # Centers the title
#                                      hjust = 0.5))
# (m1/c1) | (m4/c2) | (m5/c3) | (m2/c4) | (m3/c5)
teaser <- (m1 | m4 | m5 | m2 | m3)/(c1 | c2 | c3 | c4 | c5)
ggsave(filename = "teaser2.pdf", 
       plot = teaser,
       device = "pdf",
       width = 12,
       height = 6)
