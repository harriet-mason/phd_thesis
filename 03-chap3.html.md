...
::: {.cell}
::: {.cell-output-display}
![Alternative interpretations of how a density distribution should be drawn the input is a set of distributions, according to different software: (a) ggplot2 forms the density from the mean values, (b) ggdist puts the distribution on each observation, treating uncertainty as signal, (c) ggdibbler shows the densities for multiple samples, which puts the focus on how the density might look given the uncertainty. Which is correct might depend on the analysis, but these differences illustrate how uncertainty is treated by visualisation software.](03-chap3_files/figure-html/fig-dist-example-1.png){#fig-dist-example width=100%}
:::
:::
...
If we allow for any statistic, we will create a mismatch where the values we are trying to feed into our statistic, $S$ is not on the same domain expected by the function.
...
We can see that in the univariate case, where the intercept of the line is am $N(0,1)$ distribution the information conveyed by the sample and the quantile are very similar.
...
::: {.cell-output-display}
![No grouping](03-chap3_files/figure-html/fig-grouping-need-1.png){#fig-grouping-need-1 width=100%}
:::

::: {.cell-output-display}
![Grouped by drawID](03-chap3_files/figure-html/fig-grouping-need-2.png){#fig-grouping-need-2 width=100%}
:::
