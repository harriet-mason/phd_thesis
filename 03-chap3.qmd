...
d3 <- ggplot(data = density_data) + 
  geom_density_sample(aes(x=xdist), times=100, linewidth=0.1) +
  xlab("Height") +
  ggtitle("c. ggdibbler") +
  theme(plot.title = element_text(size = tsize))
...
```{r}
#| label: fig-meanprob
#| fig-cap: "A set of four raster plots that illustrate the problem with selecting a single valued representaion of the distribution, such as the mean. We can see that as the variance in the estimates increases, the visualisation of the sample becomes harder to read and conveys more uncertainty, but the visualisation of the mean will remain unchanged."
#| fig-height: 6
#| fig-width: 14
#| out-width: 100%
library(colorspace)
...
q1 <- qdf |>
  ...
  ggtitle("Quantile: Random Intercept") +
...
q2 <- sdf |>
  ...
  scale_x_continuous_distribution(limits = c(-2,2)) +
  scale_y_continuous_distribution(limits = c(-2,2)) +
  ggtitle("Sample: Random Intercept") +
  theme(legend.position="none")
...
q3 <- qdf |>
  ...
  scale_x_continuous_distribution(limits = c(-2,2)) +
  scale_y_continuous_distribution(limits = c(-2,2)) +
  ggtitle("Quantile: Random Intercept and Sample") +
  theme(legend.position="none")
...
q4 <- sdf |>
  ...
  scale_x_continuous_distribution(limits = c(-2,2)) +
  scale_y_continuous_distribution(limits = c(-2,2)) +
  ggtitle("Sample: Random Intercept and Sample") +
  theme(legend.position="none")
...
#| layout-ncol: 2
#| fig-cap: "An illustration of the effect of grouping on the statistic of the plot. We can see that we need to pass our samples through the visual function in batches to ensure that the statistics are not artificially changed by the sample size. "
#| fig-subcap: 
#|   - "No grouping"
#|   - "Grouped by drawID"
#| fig-height: 5
...
#| fig-align: center
#| fig-cap: "An illustration of the extensive flexibility offered by our formalisation of uncertainty visualisation. There is no limitation on visualisation type as we include uncertainty in a line, map, pie chart, histogram, bubble chart, and network diagram. There is also no limitation on aesthetics, as position, colour, size, slope, are all mapped using random variables. The implementation of this formalisation in ggdibbler, means that all these plots can be made with almost identical syntax as we would use for the deterministic ggplot2 equivalent."
#| fig-height: 6
#| fig-width: 9
#| out-width: 100%
...
pgraph <- ggraph(graph_sample, weights = weight) + 
  geom_edge_link(aes(group=drawID), position=jitter, alpha=0.005, 
                 linewidth=1, colour = "#cc3311") + 
  theme_void() +
  theme(aspect.ratio = 1) +
...
#| fig-cap: "A scatter plot of a random matrix version of the mtcars data from the 'datasets' R package, with the aesthetic mapping x=mpg, y=wt and colour=cyl. This plot shows the impact of an appropriately chosen times argument. We can see that as the sample size increases, the distributions form cohesive units and stop looking like a collection of separate points with little connection. This is not always achievable due to computational cost, but we should, at the very least, select a sample size that means our conclusions are not changing between renderings of the plot."
#| fig-height: 5
#| fig-width: 12
#| out-width: 100%
 
s1 <- ggplot(data = uncertain_mtcars) +
...
s2 <- ggplot(data = uncertain_mtcars) +
  geom_point_sample(aes(x=mpg, y=wt, 
                        colour = dist_transformed(cyl, as.factor, as.numeric)), 
