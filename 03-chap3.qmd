...
#| out-width: 100%
library(colorspace)
p0 <- ggplot(uncertain_faithfuld, aes(x = waiting, 
                                      y = eruptions, 
                                      fill = mean(density0)))+
  geom_raster() +
  ggtitle("a. mean") +
...
p1 <- ggplot(faithfuld |> mutate(density = dist_normal(density, 0.003)), aes(waiting, eruptions, fill = density)) +
  geom_raster_sample()+
  ggtitle("b. low error") +
  scale_fill_continuous_sequential(palette = "Terrain", rev = FALSE)+
  theme(legend.position = "none", legend.direction = "horizontal",
...
p2 <- ggplot(faithfuld |> mutate(density = dist_normal(density, 0.01)), aes(waiting, eruptions, fill = density))  +
  geom_raster_sample() +
  ggtitle("c. moderate error") +
  scale_fill_continuous_sequential(palette = "Terrain", rev = FALSE)+
  theme(legend.position = "none", legend.direction = "horizontal",
        plot.title = element_text(size = tsize))


p3 <- ggplot(faithfuld |> mutate(density = dist_normal(density, 0.05)), aes(waiting, eruptions, fill = density)) +
  ggtitle("d. high error") +
  geom_raster_sample() +
  scale_fill_continuous_sequential(palette = "Terrain", rev = FALSE)+
  theme(legend.position = "none", legend.direction = "horizontal", 
        plot.title = element_text(size = tsize))
...
l2 <- ggplot(uncertain_mpg, aes(displ, hwy)) +
  ...
  ggtitle("b. group includes DrawID") +
  theme(plot.title = element_text(size = tsize))
...
textdata <- expand_grid(x = c(1,2,3,4,5), y= c(1,2,3,4,5)) |>
  mutate(z0 = sample(c(TRUE, FALSE), 25, replace=TRUE)) |>
  mutate(z2 = dist_bernoulli(0.3 + 0.4*z0)) 
...
p7 <- ggplot(tiledata, aes(x=x, y=y)) +
  ...
  geom_tile(colour="white", fill=NA, linewidth=1.5) +
...
#| fig-align: center
#| fig-cap: "This formalisation of uncertainty visualisation offers extensive flexibility, illustrated by six plots: line, pie chart, histogram, map, bubble chart, and network diagram. These plots are made with almost identical syntax with `ggdibbler` as that of the deterministic `ggplot2` equivalent. These aesthetics - position, colour, size, slope - are all mapped using random variables."
#| fig-height: 6
#| fig-width: 15
#| out-width: 100%
#| dev: "png"
